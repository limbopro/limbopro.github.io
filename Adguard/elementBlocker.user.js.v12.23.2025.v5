...
`;
        ...
        if (hintBtn && outputField) {
            hintBtn.onclick = () => {
                ...
                // 修正后的正则：匹配字符串中最后一个出现的 :nth-child(n)
                const lastNthRegex = /:nth-child\(\d+\)(?!.*:nth-child)/;

                if (lastNthRegex.test(currentSelector)) {
                    // 1. 执行删除
                    const newSelector = currentSelector.replace(lastNthRegex, '');

                    // 2. 更新 UI 显示
                    outputField.innerText = newSelector;

                    // 【同步执行高亮】
                    SelectorBlockerTool.applyPreview(newSelector);

                    // 3. 触发预览高亮 (使用你习惯的 !important 逻辑)
                    if (typeof this.applyPreview === 'function') {
                        this.applyPreview(newSelector);
                    }
                    ...
                    try {
                        ...
                        if (tipEl && count > 1) {
                            ...
                            if (msgEl) msgEl.innerText = "范围已扩大";
                        }
                    } catch (e) {
                        console.warn("手动编辑可能导致了语法错误");
                    }
                } else {
                    // 反馈：已经没有索引可以删除了
                    hintBtn.innerText = "已无更多索引";
