...
// =================================================================
// 元素屏蔽/追踪器 完整脚本 V25.1 (Iframe支持, click模式, 恢复 body_build)
// =================================================================
...
function renderFloatWindow(targetDocs) {
    ...
    function toggleSelectionMode(forceState) {
        ...
        targetDocs.forEach(doc => {
            if (isSelectionMode) {
                // 关键：在所有文档的捕获阶段绑定 click
                doc.addEventListener('click', handleSelectionClick, true);
                doc.addEventListener('mousemove', handleSelectionMouseMove); // mousemove 绑定在当前文档
            } else {
    ...
    // --- 列表逻辑 (使用 element.document 确保高亮正确) ---
    list.addEventListener('click', (e) => {
        let listItem = e.target.closest('li');
        if (!listItem) return;

        const xpath = listItem.getAttribute('data-xpath');
        // 必须从 zeroOpacityElements 中查找元素对象，因为它包含 document 引用
