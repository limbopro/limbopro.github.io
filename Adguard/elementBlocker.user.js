        ...
        if (simplifyBtn && outputField) {
            simplifyBtn.onclick = () => {
                ...
                for (let i = 0; i < allNths.length; i++) {
                    ...
                    if (checkUnique(testSelector)) {
                        activeIndices = testIndices;
                        tempSelector = testSelector;
                    } else { break; } // 按照要求，左侧遇到关键索引即停止
                }

                // 【步骤 B：从右向左索引精简】(保留最后一个索引)
                if (activeIndices.length > 1) {
                    for (let j = activeIndices.length - 2; j >= 0; j--) {
                        let targetIdx = activeIndices[j];
                        let testIndices = activeIndices.filter(idx => idx !== targetIdx);
                        let testSelector = rebuildByIndices(currentSelector, testIndices);
                        if (checkUnique(testSelector)) {
                            activeIndices = testIndices;
                            tempSelector = testSelector;
                ...
                // 【步骤 C：层级路径剥离】(移除左侧冗余父级)
                let segments = tempSelector.split(/\s*>\s*/);
                if (segments.length > 1) {
                    let finalPath = tempSelector;
                    for (let k = 0; k < segments.length - 1; k++) {
                        let testSelector = segments.slice(k + 1).join(' > ');
                        if (checkUnique(testSelector)) {
                            finalPath = testSelector;
                        } else { break; } // 左侧层级是必要的
                    }
                    tempSelector = finalPath;
                }
                ...
                simplifyBtn.innerText = "已极致精简";
                setTimeout(() => { simplifyBtn.innerText = "极致精简"; }, 1500);
            };
        }
