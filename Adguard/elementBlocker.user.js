        ...
        return new Promise((resolve) => {
            ...
            modalBox.innerHTML = `
                ...
                <div class="operation-notes" style="margin-bottom: 20px;">
                    ...
                    ">
                        <strong>🛡️ [永久屏蔽]</strong> 将此元素永久添加到屏蔽列表并移除。(基于xPath屏蔽)
                    </p>
                    <p style="
                        font-size: 13px; padding: 5px 10px; 
        ...
        saveButton.onclick = () => {
            ...
            selectors.forEach(sel => {
                ...
                try {
                    ...
                    confirmndExecuteFC(`[Gemini屏蔽 V27] 语法错误: ${sel}`)
                }
            ...
            if (savedCount > 0) {
                confirmndExecuteFC(`✅ 成功保存 ${savedCount} 个 CSS 选择器！是否刷新页面生效？`, () => { location.reload() })
            } else {
    ...
    window.startSelectorTool = function () {
        ...
        window.getSmartSelector = function getSmartSelector(el) {
            ...
            while (current) {
                ...
                let index = 1, sib = current.previousElementSibling;
                while (sib) { index++; sib = sib.previousElementSibling; }
                if (current.parentElement && current.parentElement.children.length > 1) {
                    segment += `:nth-child(${index})`;
                }
                if (current.classList.length > 0) {
                    segment += "." + Array.from(current.classList).map(c => CSS.escape(c)).join(".");
                }
                path.unshift(segment);
                if (path.length >= 5 || ['body', 'html'].includes(current.nodeName.toLowerCase())) break;
...
.sel-result-window { 
    ...
    z-index: 114119; width: 90%; max-width: 420px; 
...
.sel-title { 
    ...
    cursor: move !important; padding: 10px 0 !important;
...
.sel-actions { 
    margin-top: 15px !important; display: flex !important; flex-wrap: wrap !important; gap: 10px !important; 
}

.sel-btn { 
    flex: 1; min-width: 70px; min-height: 44px; /* 移动端大点击区域 */
    padding: 8px !important; border: none !important; border-radius: 8px !important; 
    cursor: pointer !important; font-weight: bold !important; font-size: 13px !important;
}
...
@media (max-width: 600px) {
    .sel-result-window { top: auto; bottom: 40px; }
}
`;
        document.head.appendChild(style);
        ...
        resultWin.innerHTML = `
        <span class="sel-title">精准 CSS Selector 结果:</span>
        <code class="sel-code" id="sel-output"></code>
        <div class="sel-actions">
            <button class="sel-btn sel-copy-btn" id="sel-copy">复制</button>
            <button class="sel-btn" style="background:#f39c12" id="sel-block">屏蔽</button>
    ...
    `;
        ...
        // 2. 绑定“屏蔽”按钮点击事件
        document.getElementById('sel-block').onclick = () => {
            const sel = document.getElementById('sel-output').innerText;
            ...
            // --- 你提供的屏蔽逻辑核心 ---
            try {
                // 使用 DocumentFragment 验证 CSS 语法合法性，无需元素真实存在
                document.createDocumentFragment().querySelector(sel);

                // 调用你的存储/屏蔽函数 (请确保 saveCssRemovalChoice 已定义)
                if (typeof saveCssRemovalChoice === 'function') {
                    if (saveCssRemovalChoice(sel)) {
                        ...
                        confirmndExecuteFC(`${sel} 已屏蔽...`)
                    }
                } else {
                    console.error('[错误] 找不到 saveCssRemovalChoice 函数');
                    // 如果函数未定义，至少在控制台给你个提示
                    console.log(`拟屏蔽的选择器: ${sel}`);
                    confirmndExecuteFC(`${sel} 已屏蔽...`)
                }
            } catch (e) {
                confirmndExecuteFC('生成的选择器语法有误，无法屏蔽。')
                console.warn(`[Gemini屏蔽 V27] 语法错误: ${sel}`);
        ...
        const onDragStart = (e) => {
            ...
            // --- 关键：锁定宽高并清除所有定位锚点，防止拉伸 ---
            Object.assign(resultWin.style, {
                width: rect.width + 'px',   // 锁定当前像素宽度
                height: rect.height + 'px', // 锁定当前像素高度
                left: rect.left + 'px',
                top: rect.top + 'px',
                bottom: 'auto',             // 必须清除 bottom，防止向上拖拽时被拉伸
                right: 'auto',              // 必须清除 right
                transform: 'none',          // 清除居中的 translate
                margin: '0'
            });
        ...
        const onDragMove = (e) => {
            ...
            // 边界限制：防止拖出屏幕
            const maxX = window.innerWidth - resultWin.offsetWidth;
            const maxY = window.innerHeight - resultWin.offsetHeight;
            newX = Math.max(0, Math.min(newX, maxX));
            newY = Math.max(0, Math.min(newY, maxY));

            resultWin.style.left = newX + 'px';
            resultWin.style.top = newY + 'px';
        };
...
<button id="manual-css-webdebug" style="font-size:xx-small;background:#6c1a7bff; color: white; border: none; padding: 8px 15px; cursor: pointer; border-radius: 4px; font-weight: bold; width: 100%;" onclick="window.initWebDebugger()"> ⚙️ Web 存储调试器
    ...
    ▶️启动清理透明元素🔴
            ...
            <div class="gemini-tip-text">
                🌟**提示:** “选择并屏蔽模式”及🛠️ 元素点击调试中的屏蔽原理基于<a href='https://www.google.com/search?q=xpath+%E6%98%AF%E4%BB%80%E4%B9%88' target='_blank' style='corlor:blue;'>xPath</a>。⛔标记为黑名单页意味着对当前页面存在的所有iframe进行<a href='https://www.google.com/search?q=iframe+sandbox%E5%B1%9E%E6%80%A7' target='_blank' style='corlor:blue;'>沙箱化</a>处理，严格限制其交互权限。
            </div>
        `;
        ...
        const eventListenerFunction = async (e) => {
            ...
            if (shouldIntercept && targetElement.tagName !== 'HTML' && targetElement.tagName !== 'BODY') {
                ...
                const elementInfo = {
                    ...
                    preciseSelector: getSmartSelector_w(targetElement),   // 包含 nth-child 的精确选择器 [新属性]
                };
