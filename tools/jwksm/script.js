    ...
    (function () {
        ...
        `;
            ...
            if (currentLength > baseline) {
                ...
                document.getElementById('yesterday').innerText = "；自您首次打开本页面以来（30天内），新增" + (currentLength - baseline) + "部"; // 改变说明
            } else if (currentLength < baseline) {
                comparisonMessage += `\n结果: length **减少了** ${baseline - currentLength}。`;
                document.getElementById('yesterday').innerText = "；自您首次打开本页面以来（30天内），" + (currentLength - baseline) + "部";  // 改变说明
            } else {
                comparisonMessage += `\n结果: length **与基准组值保持一致**。`;
            }
